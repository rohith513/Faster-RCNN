...
WEIGHT_RGLZ = regularizers.l2(1e-4)
BIAS_RGLZ = regularizers.l2(1e-4)
...
def identity_block(input_tensor, kernel_size, filters, stage, block, trainable=True):
    ...
    x = Conv2D(nb_filter1, (1, 1), name=conv_name_base + '2a', trainable=trainable, kernel_regularizer=WEIGHT_RGLZ, bias_regularizer=BIAS_RGLZ)(input_tensor)
    x = FixedBatchNormalization(axis=bn_axis, name=bn_name_base + '2a')(x)
    x = Activation('relu')(x)
        
    x = Conv2D(nb_filter2, (kernel_size, kernel_size), padding='same', name=conv_name_base + '2b', trainable=trainable, kernel_regularizer=WEIGHT_RGLZ, bias_regularizer=BIAS_RGLZ)(x)
    x = FixedBatchNormalization(axis=bn_axis, name=bn_name_base + '2b')(x)
    x = Activation('relu')(x)
        
    x = Conv2D(nb_filter3, (1, 1), name=conv_name_base + '2c', trainable=trainable, kernel_regularizer=WEIGHT_RGLZ, bias_regularizer=BIAS_RGLZ)(x)
...
def rpn(base_layers,num_anchors):
    ...
    x = Conv2D(512, (3, 3), padding='same', activation='relu', kernel_initializer='normal', kernel_regularizer=WEIGHT_RGLZ, bias_regularizer=BIAS_RGLZ, name='rpn_conv1')(base_layers)

    gaussian_initializer = TruncatedNormal(stddev=0.01)
    x_class = Conv2D(num_anchors, (1, 1), activation='sigmoid', kernel_initializer=gaussian_initializer, kernel_regularizer=WEIGHT_RGLZ, bias_regularizer=BIAS_RGLZ, name='rpn_out_class')(x)
    x_regr = Conv2D(num_anchors * 4, (1, 1), activation='linear', kernel_initializer=gaussian_initializer, kernel_regularizer=WEIGHT_RGLZ, bias_regularizer=BIAS_RGLZ, name='rpn_out_regress')(x)
...
def classifier(base_layers, input_rois, num_rois, nb_classes = 2, trainable=False):
    ...
    out_class = TimeDistributed(Dense(nb_classes, activation='softmax', kernel_initializer=gaussian_initializer_cls, kernel_regularizer=WEIGHT_RGLZ, bias_regularizer=BIAS_RGLZ), name='dense_class_{}'.format(nb_classes))(out)
    # note: no regression target for bg class
    out_regr = TimeDistributed(Dense(4 * (nb_classes-1), activation='linear', kernel_initializer=gaussian_initializer_regr, kernel_regularizer=WEIGHT_RGLZ, bias_regularizer=BIAS_RGLZ), name='dense_regress_{}'.format(nb_classes))(out)
